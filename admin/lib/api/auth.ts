...
import { AddAdmin, AllUser, authRegsiter, User, UserApiResponse, UserBase, UserCreate } from "../types/auth";
...
export async function add_User(user:UserBase):Promise<void>{
   const userCr :UserCreate={
    ...user,
    "specialite":"pharmacie",
    "study_Year":"2005"
   }
   await client.post(API_ENDPOINTS.AUTH.ROOT , userCr);
