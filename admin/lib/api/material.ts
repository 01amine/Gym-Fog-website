...
export async function deleteMaterialById(id : string): Promise<void> {
    await client.delete(API_ENDPOINTS.MATERIALS.BY_ID(id));
}
...
export async function editMaterialById({
  ...
  if (data.study_year !== undefined) {
    data.study_year.forEach(year => {
      formData.append('year_study', year);  
    });
  }

  if (data.specialite !== undefined) {
    data.specialite.forEach(spec => {
...
export async function createMaterial(
  ...
  data.study_year.forEach(year => {
    formData.append('study_year', year);
  });
