...
import { add_User, addAdmin, block_user, get_me, get_user_by_name, getAllUsers, getStudents, getUserbyId, login, logout, removeAdmin, unblock_user } from "@/lib/api/auth";
import { AllUser, authRegsiter, User, UserBase } from "@/lib/types/auth";
...
const queryClient = useQueryClient()
return useMutation({
  mutationFn:(user:UserBase )=>add_User(user),
  onSuccess:()=>{
    queryClient.invalidateQueries({queryKey:["users"]})
  }
})
